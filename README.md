...
外部 SymPy 执行 v1 已开始接入性质分析自检：当内置轻量复算无法处理但能整理成安全表达式时，runner 会通过受限 Python/SymPy wrapper 复算偏导，并将不一致结果纳入同一个有边界修复闭环。该 wrapper 有输入长度、字符范围和超时限制；如果部署环境没有 Python/SymPy，系统会把该项标为人工复核，不会让产品流程崩溃。当前外部 SymPy 只用于性质分析偏导复算，还没有替代均衡求解器，也不会执行模型生成的任意 Python 代码。
