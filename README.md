...
3. CAS/SymPy v2：扩大数学复算范围，并明确区分已通过、需修正、条件不足、暂不支持和人工复核；第一步已接入受限外部 SymPy 偏导复算、显式 FOC 残差复核、显式 FOC 独立求解对照，以及从安全结构化利润函数自动生成 FOC 残差的均衡复核入口；后续继续推进更完整的内点/边界分类和多方程求解诊断。
