...
外部 SymPy 执行 v1 已开始接入均衡和性质分析自检：当内置轻量复算无法处理但能整理成安全表达式时，runner 会通过受限 Python/SymPy wrapper 复算偏导；当均衡候选给出可执行 FOC 残差时，runner 会把闭式解代回 FOC，检查残差是否为 0。不一致结果会纳入同一个有边界修复闭环。该 wrapper 有输入长度、字符范围和超时限制；如果部署环境没有 Python/SymPy，系统会把该项标为人工复核，不会让产品流程崩溃。当前外部 SymPy 仍不是完整均衡求解器，也不会执行模型生成的任意 Python 代码。
...
3. CAS/SymPy v2：扩大数学复算范围，并明确区分已通过、需修正、条件不足、暂不支持和人工复核；第一步已接入受限外部 SymPy 偏导复算，后续继续推进均衡求解执行器。
