...
“均衡最优性验证 v1”已接入求解内核：系统会在 FOC 残差和独立求解对照之后记录 `second_order_conditions`、`hessian_check`、`concavity_check` 和 `boundary_kkt_check` 数学产物。当前只对安全的简单二次利润表达式做可执行二阶导符号判断；不同玩家各自一维决策时可用二阶条件作为最优性证据，同一玩家多决策、复杂 Hessian、参数依赖二阶项和边界/KKT 情况会转入人工复核或条件不足，不会被标成已通过。FOC 通过但二阶导为正会进入均衡候选修复；候选位于非负约束等边界且缺少 KKT/边界区域分析时，会停在人工复核而不是生成正式均衡 patch。这个阶段仍不是通用 CAS/Hessian/KKT 求解器，目标是防止把一阶驻点误当成利润最大化均衡。

“Solver v3 编译层”已开始接入 `compiled_game_system` 数学产物：系统会把确认模型拆成玩家、战略变量、状态/需求变量、参数、约束、时序、目标函数、可生成 FOC、最优性义务、有限求解策略和失败分类。策略元数据覆盖线性系统、反应函数、显式 FOC 求解、残差代入和隐式系统兜底；失败分类会给出模型缺口、暂不支持表达式等下一步动作。该层只生成受限代数输入和审计元数据，仍然通过既有受限 SymPy wrapper 做实际复核，不执行模型生成的任意 Python 代码。
...
- 已完成 v1：性质分析 runner 已接入受限外部 SymPy 偏导复算；轻量验证器处理不了但可安全交给 SymPy 的表达式，如果复算不一致，会被视为自检问题进入同一个修复闭环。
- 已完成 v1：均衡 runner 已接入受限外部 SymPy FOC 残差复核；候选闭式解若无法满足可执行 FOC 残差，会被视为自检问题进入同一个修复闭环。
- 已完成 v2：均衡 runner 已接入受限外部 SymPy 独立求解对照；显式 FOC 系统会调用 `sympy.solve`，并确认候选闭式解是否出现在系统求解结果中，不匹配时进入同一个修复闭环。
- 已完成 v2：均衡 runner 已接入安全结构化利润函数的 FOC 生成入口；候选 FOC 不可执行时，会尝试从 `model.profitFunctions` 对候选闭式解变量求导生成残差，再复核闭式解，而不是只停在人工复核。
