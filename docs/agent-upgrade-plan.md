...
3. CAS/SymPy v2：已将数学验证结果结构化为通过、失败、条件不足、暂不支持和人工复核；当前新增受限外部 SymPy 偏导复算、FOC 残差复核和显式 FOC 独立求解对照入口，但尚未把外部 SymPy 扩展为任意模型的完整均衡求解器。
