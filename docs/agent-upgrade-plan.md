...
- 待加强：模型风险检查目前是轻量规则，后续可扩展符号冲突、参数冗余、均衡可解性和比较静态目标检查。
...
- 待加强：当前自检主要覆盖 solved 状态、闭式表达、FOC、求解步骤和存在条件；后续可扩展内点/边界分类、SymPy 复算、失败诊断和模型简化 patch 建议。
...
- 待加强：当前自检主要覆盖命题数量、重复 id、分析对象、参数、符号结果、符号条件、命题草稿和证明草图；后续可扩展与均衡表达式的符号引用一致性、条件强弱检查和命题去重合并。
