...
- “需修正”表示系统发现数学关系不一致，不要继续生成论文输出作为正式结果。
- “条件不足”表示需要补模型假设、参数正负、取值区间或均衡存在条件后再重跑。
- “人工复核”不是失败，也不是已证明；维护者应提醒研究者展开复核说明，看跳过原因、残差结果或 solver attempt 记录后再决定是否继续。
- 右侧数学验证面板应该显示处理方式，并提供跳转到模型设定、符号均衡和性质分析的入口；如果只有计数没有处理指引，应暂停上线并回归 `math-verification-summary` / `research-assets-panel`。
- 若问题来自后台任务，检查 `agent_tasks.checkpoints` 和右侧任务审计面板中的数学产物快照，确认 `compiled_game_system`、`generated_foc_system`、`sympy_residual_check`、`solver_attempt` 或 `sympy_solve_check` 是否被正确保存。

论文预览公式渲染异常：

- 优先检查草稿内容是否有不成对的 `$`、`$$`、`\(`、`\[` 分隔符。
- 如果块级公式被拆成奇怪的行内公式，回归 `src/lib/markdown-math.ts` 和 `src/components/markdown-renderer.test.mjs`；渲染器必须保护已有 `$$...$$` 块，不应在块内再次包裹裸符号。
- 导出 Markdown 后也要打开预览，确认 KaTeX 公式、代码块和可复核 SymPy 脚本没有互相污染。
